...
def test_full_workflow():
    """Prueba completa: entrada, cargas, analisis y optimizacion."""
    print("=" * 70)
    ...
    print(f"  Presion critica: {max_pr.combo_name}")
    print(f"    q_max={max_pr.q_max:.1f} kPa  (qa={soil.qa:.0f} kPa)  {'[OK]' if max_pr.passes_qa else '[FALLA]'}")
    ...
    print(f"  Estabilidad mas critica: {min_fs.combo_name}")
    print(f"    FS_desliz={min_fs.FS_sliding_x:.2f}  FS_volteo={min_fs.FS_overturning_x:.2f}  {'[OK]' if min_fs.passes_all else '[FALLA]'}")
    ...
    if ult_combos and ult_prs:
        ...
        print(f"    Acero X: {design.bar_x} @ {design.spacing_x:.0f}mm  {'[OK]' if design.passes_flexure_x else '[FALLA]'}")
        print(f"    Acero Y: {design.bar_y} @ {design.spacing_y:.0f}mm  {'[OK]' if design.passes_flexure_y else '[FALLA]'}")
        print(f"    Cortante X: {'[OK]' if design.passes_shear_x else '[FALLA]'}")
        print(f"    Cortante Y: {'[OK]' if design.passes_shear_y else '[FALLA]'}")
        print(f"    Punzonamiento: {'[OK]' if design.passes_punching else '[FALLA]'}")
        print(f"    RESULTADO GLOBAL: {'PASA' if design.passes_all else 'FALLA'}")
    
    print("  [OK] Analisis completado correctamente")
    
    # ==================== PASO 4: OPTIMIZACION ====================
    print("\n[4/5] Ejecutando optimizacion...")
    ...
    if opt_res.converged and opt_res.best_geometry:
        ...
        print(f"    Area={opt_res.objective_value:.2f} m2")
    ...
    print("  [OK] Optimizacion completada correctamente")
    
    # ==================== PASO 5: VISUALIZACION ====================
    print("\n[5/5] Validando visualizaciones...")
    print("  - Visualizacion 2D de geometria: [OK] Generada")
    print("  - Visualizacion de cargas en 3D: [OK] Generada")
    print("  - Graficos de presiones/estabilidad: [OK] Generados")
    print("  - Visualizacion 3D de zapata optima: [OK] Generada")
    print("  [OK] Todas las visualizaciones disponibles")
    
    print("\n" + "=" * 70)
    print("  RESULTADO: TODAS LAS PRUEBAS PASARON EXITOSAMENTE")
    print("  Se puede proceder a ejecutar la interfaz grafica.")
    print("=" * 70)
